...
describe('CoursesService', () => {
  ...
  it('should find a course by id', () => {
    coursesService.findCourseById(4)
      .subscribe(course => {
        expect(course).toBeTruthy('No course returned');
        expect(course.id).toBe(4);
        expect(course.titles.description).toBe("NgRx In Depth");
        expect(course.lessonsCount).toBe(10);
      });

      const req = httpTestingController.expectOne('/api/courses/4');
      expect(req.request.method).toEqual('GET');
  ...
  it('should give an error if save course fails', () => {
    ...
    coursesService.saveCourse(12, changes)
      .subscribe(
        ...
        error => {
          expect(error).toBeTruthy('No error message');
